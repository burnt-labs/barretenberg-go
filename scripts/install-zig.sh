...
EOF
cat > "$INSTALL_DIR/bin/zig-cc" <<'EOF'
#!/usr/bin/env sh
exec "$(dirname "$0")/../zig" cc "$@"
EOF
cat > "$INSTALL_DIR/bin/zig-c++" <<'EOF'
#!/usr/bin/env sh
exec "$(dirname "$0")/../zig" c++ "$@"
EOF
cat > "$INSTALL_DIR/bin/zig-ar" <<'EOF'
#!/usr/bin/env sh
exec "$(dirname "$0")/../zig" ar "$@"
EOF
chmod +x "$INSTALL_DIR/bin/aarch64-linux-musl-zig-cc" \
    "$INSTALL_DIR/bin/aarch64-linux-musl-zig-c++" \
    "$INSTALL_DIR/bin/zig-cc" \
    "$INSTALL_DIR/bin/zig-c++" \
    "$INSTALL_DIR/bin/zig-ar"
