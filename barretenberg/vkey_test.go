...
// loadVkeyTestVector loads the vkey test vector from testdata; skips test if missing.
func loadVkeyTestVector(t *testing.T) []byte {
	t.Helper()
	path := filepath.Join(vkeyTestdataDir, vkeyTestFile)
	data, err := os.ReadFile(path)
	if err != nil {
		t.Skipf("test vector %s not found: %v", vkeyTestFile, err)
	}
	return data
}
